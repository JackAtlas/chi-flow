...
import type { WorkflowExecutionGetPayload } from '@/generated/prisma/models'
import type { ExecutionPhase } from '@/generated/prisma/client'
...
type ExecutionWithPhases = WorkflowExecutionGetPayload<{
  include: { phases: true }
}>

async function initializePhasesStatus(execution: ExecutionWithPhases) {
...
async function decrementCredits(
  ...
  try {
    await prisma.userBalance.update({
      where: { userId, credits: { gte: amout } },
      data: { credits: { decrement: amout } }
    })
    return true
  } catch {
