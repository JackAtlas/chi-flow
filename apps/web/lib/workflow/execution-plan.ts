    ...
    for (const currentNode of nodes) {
      ...
      if (invalidInputs.length > 0) {
        ...
        if (incomers.every((incomer) => planned.has(incomer.id))) {
          ...
          console.warn('Invalid inputs', currentNode.id, invalidInputs)
          inputsWithErrors.push({
...
function getInvalidInputs(node: AppNode, edges: Edge[], planned: Set<string>) {
  ...
  for (const input of inputs) {
    ...
    const inputValueProvided = inputValue?.length && inputValue.length > 0
    if (inputValueProvided) {
      // Value of this input is provided by user,
    ...
    if (requiredInputProvidedByVisitedOutput) {
      ...
      if (inputLinkedToOutput && planned.has(inputLinkedToOutput.source)) {
        // The output is providing a value to the input, this input is fine
        continue
      }
    }
